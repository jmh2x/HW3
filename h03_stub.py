...
if __name__ == "__main__":
    ...
    def create_custom_colormap():
        ...
        cmap_full = LinearSegmentedColormap.from_list("custom_full", colors,
        N=n_bins)
        # Create a colormap with black for zero and full spectrum for non-zero
        colors_with_black = np.vstack((np.array([0, 0, 0, 1]),
        cmap_full(np.linspace(0, 1, 255))))
    ...
    def plot_weight_images(W):
        ...
        for i in range(10):
            ...
            cbar.set_ticklabels([f'-{abs_max:.2f}', f'-{abs_max/2:.2f}', '0',
            f'{abs_max/2:.2f}', f'{abs_max:.2f}'])
        fig.suptitle('Pixel weights in trained W for each class', fontsize=16,
        y=1.05)
        plt.tight_layout()
        plt.show()
    ...
    overlap_results, W_zero_pixels = analyze_zero_overlap(W, mnist_zero_pixels,
    zero_threshold = 1e-2)
